...
pub fn gouraud(
    ...
    let mut color_b = get_illumination(&normals[0]);
    let mut color_m = get_illumination(&normals[1]);
    let mut color_t = get_illumination(&normals[2]);
...
pub fn phong(
    ...
    while y <= y_end {
        ...
        for x in x_start..=x_end {
            ...
            picture.plot(x, y, z, &get_illumination(&[nx, ny, nz]));
